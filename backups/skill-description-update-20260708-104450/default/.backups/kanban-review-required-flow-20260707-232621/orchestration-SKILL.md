...
5. **Leaving review-required blocks unresolved.** Implementation workers may block with `review-required` after successful verification. Inspect the handoff, then either complete the parent to promote dependent QA, or send explicit feedback and keep it blocked.
...
- [ ] Blocked tasks were classified correctly: dependency wait, review-required handoff, or worker crash.
