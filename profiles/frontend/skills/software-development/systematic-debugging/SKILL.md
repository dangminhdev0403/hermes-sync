...
**For frontend → API validation failures and proxy-route bugs:**
- Read the backend response body before editing; messages like `Unrecognized key` usually identify an exact payload contract mismatch.
- Trace the payload construction at the client boundary, not only the route handler. Distinguish request body fields from headers/query params.
- If a value is metadata such as locale/language, confirm whether the backend expects it in headers (`Accept-Language`, `x-lang`), query, or body; do not send it in multiple places unless the contract says so.
- For Next.js proxy routes, check both the app route sanitization and the service/client class that ultimately performs the HTTP request.
- Verify the proxy route maps to the intended backend endpoint. A frontend path like `/api/.../stays` can call a semantically different backend action such as `/stays/check-in`, so inspect the route handler and service method before assuming the URL name describes the side effects.

**For workflow/side-effect bugs:**
- Trace where side effects are introduced across UI route → internal API route → service client → backend controller → domain service → repository. Fix the layer that actually creates the side effect, not the button or label that exposes it.
- When a user says an operation happens at the wrong lifecycle stage, preserve explicit management actions and remove implicit side effects from lifecycle operations unless the domain contract says they belong there.
